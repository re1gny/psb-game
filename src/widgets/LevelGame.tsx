      ...
      <div className={'relative h-full'} onClick={handleClick}>
        <div className={`absolute top-0 left-0 w-full h-full transition-[filter] duration-200 ${hasBlur ? 'blur pointer-events-none' : ''}`}>
          <img 
            className={'absolute'} 
            style={{
              top: `${level.bgTop1.y}px`,
              left: `${level.bgTop1.x}px`,
              width: `${level.bgTop1.width}px`,
              height: `${level.bgTop1.height}px`,
            }}
            src={level.bgTop1.image} alt="" />
          <img 
            className={'absolute'} 
            style={{
              top: `${level.bgTop2.y}px`,
              left: `${level.bgTop2.x}px`,
              width: `${level.bgTop2.width}px`,
              height: `${level.bgTop2.height}px`,
            }}
        ...
        <AnimatePresence initial={false}>
          {!cardsSelectorOpened && (
            <motion.button
              ...
              className={`absolute top-[20px] left-[20px] flex items-center justify-center border-2 border-[#000000] rounded-[10px] bg-[#EA5616] w-[48px] h-[48px] transition-[filter] duration-200 ${hasBlur && !hasBaseBlur ? 'blur pointer-events-none' : 'z-20'}`}
              onClick={back}
            >
        ...
        <AnimatePresence initial={false}>
          {!cardsSelectorOpened && (
            <motion.button
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className={`absolute top-[20px] right-[20px] flex items-center justify-center border-2 border-[#000000] rounded-[10px] bg-[#EA5616] w-[48px] h-[48px] transition-[filter] duration-200 ${hasBlur && !hasBaseBlur ? 'blur pointer-events-none' : 'z-20'}`}
        ...
        <LivesScore 
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={`absolute top-[20px] left-1/2 -translate-x-1/2 transition-[filter] duration-200 ${hasBlur ? 'blur pointer-events-none' : 'z-20'}`} 
        ...
        <div className={'absolute top-0 left-0 w-full h-full z-0'}>
          <AnimatePresence initial={false}>
            <motion.div
              ...
              style={{
                width: `${level.monsterImages[monsterImageIndex].width}px`,
                height: `${level.monsterImages[monsterImageIndex].height}px`,
                top: `${level.monsterImages[monsterImageIndex].y}px`,
                left: `${level.monsterImages[monsterImageIndex].x}px`,
              }}
            ...
            {selectedCards.map((card) => {
              ...
              return (
                <motion.div
                  ...
                  style={{
                    top: `${config[card.id][pairIndex].y}px`,
                    left: `${config[card.id][pairIndex].x}px`,
                    width: `${config[card.id][pairIndex].width}px`,
                    height: `${config[card.id][pairIndex].height}px`,
                    zIndex: pairIndex + 1,
                  }}
                >
                  <img
        ...
        <AnimatePresence>
            ...
            (winnable === true && clicks === index + 1) && (
              <motion.div
                ...
                style={{
                  ...
                  top: `${task.y}px`,
                  left: `${task.x}px`,
                  padding: task.padding.map(item => `${item}px`).join(' ')
                }}
              >
                <img
                ...
                <p
                  ...
                  style={{
                    width: `calc(100% - ${task.padding[1] + task.padding[3]}px)`,
                    height: `calc(100% - ${task.padding[0] + task.padding[2]}px)`
                  }}
                >
                  {task.text}
                </p>
          ...
          {(winnable === false && (step === 7 || step === 8) && clicks > clicksRequired) && (
            <motion.div
              ...
              className={'absolute z-0 flex items-start justify-center pointer-events-none'}
              style={{
                top: `${level.looseMonsterText.y}px`,
                left: `${level.looseMonsterText.x}px`,
              }}
            >
              <svg
                style={{
                  width: `${level.looseMonsterText.width}px`,
                  height: `${level.looseMonsterText.height}px`,
                }}
        ...
        <AnimatePresence>
          {step === 2 && (
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.8 }}
              className={'flex flex-col items-center absolute top-[87px] left-[20px] w-[calc(100%-40px)]'}
            >
              <MonsterCardBack
                height={493}
                width={304.83}
        ...
        <AnimatePresence>
          {step === 7 && clicks === 0 && (
            ...
            >
              ...
              <svg className={'absolute top-[-2px] left-[252px] -translate-y-full pointer-events-none'} width="16" height="83" viewBox="0 0 16 83" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <AnimatePresence>
          {winnable === true && step === 9 && (
            ...
            >
              ...
              <Button className={'mt-[20px]'} onClick={winContinue}>
                Далее
              </Button>
            </motion.div>
          )}
        ...
        <AnimatePresence>
          {winnable === false && step === 9 && (
            ...
            >
              ...
              <Button className={'mt-[20px]'} onClick={looseContinue}>
                Попробовать снова
              </Button>
            </motion.div>
          )}
